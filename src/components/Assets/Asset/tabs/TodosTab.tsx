...
export function TodosTab({ asset, locale, onUpdateAsset }: TodosTabProps) {
  ...
  const deleteTodo = async (todoId: number) => {
    try {
      ...
      onUpdateAsset({
        ...asset,
        todos: asset.todos.filter(todo => todo.id !== todoId),
      });
    } catch (error) {
  ...
  const getObjectiveName = (objectiveId: number | null) => {
    ...
    const objective = asset.objectives.find(obj => obj.id === objectiveId);
    return objective ? objective.name : '-';
  };

  const getFilteredTodos = () => {
    return asset.todos.filter((todo) => {
