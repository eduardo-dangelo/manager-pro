...
export function ReportTab({ asset }: ReportTabProps) {
  const t = useTranslations('Assets');

  // Calculate objectives completion
  const objectivesCompleted = asset.objectives.filter(obj => obj.status === 'completed').length;
  const objectivesTotal = asset.objectives.length;
  ...
  const tasksByStatus = [
    {
      name: t('kanban_todo'),
      value: asset.todos.filter(task => task.status === 'todo').length,
      color: COLORS.todo,
    },
    {
      name: t('kanban_in_progress'),
      value: asset.todos.filter(task => task.status === 'in-progress').length,
      color: COLORS['in-progress'],
    },
    {
      name: t('kanban_done'),
      value: asset.todos.filter(task => task.status === 'done').length,
      color: COLORS.done,
    },
  ...
  const tasksByPriority = [
    {
      name: 'Low',
      value: asset.todos.filter(task => task.priority === 'low').length,
      fill: COLORS.low,
    },
    {
      name: 'Medium',
      value: asset.todos.filter(task => task.priority === 'medium').length,
      fill: COLORS.medium,
    },
    {
      name: 'High',
      value: asset.todos.filter(task => task.priority === 'high').length,
      fill: COLORS.high,
    },
    {
      name: 'Urgent',
      value: asset.todos.filter(task => task.priority === 'urgent').length,
      fill: COLORS.urgent,
    },
  ].filter(item => item.value > 0);
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} md={4}>
          ...
          >
            ...
            >
              {asset.todos.length > 0
                ? Math.round(
                    (asset.todos.filter(t => t.status === 'done').length / asset.todos.length)
                    * 100,
                  )
                : 0}
              %
            ...
            <Typography
              ...
              sx={{ color: 'grey.500', fontSize: '0.813rem' }}
            >
              {asset.todos.filter(t => t.status === 'done').length}
              {' '}
              of
              {' '}
              {asset.todos.length}
